...
describe('order module', () => {
  ...
  let token;
  let tokenSuperAdmin;
  const strQuery = `mutation removeSnrRecord($inputResetOrder: InputResetOrder!){
    removeSnrRecord(input: $inputResetOrder){
      message
      statusCode
      error{
       errorCode
  ...
  const path = '/graphql';
  describe('remove Snr records', () => {
    it('TC_1: should be set hulft number of snr file of orders in date  to 99999', async () => {
      await request(app.getHttpServer())
        ...
        .expect(async ({ body }) => {
          const { statusCode } = body.data.removeSnrRecord;
